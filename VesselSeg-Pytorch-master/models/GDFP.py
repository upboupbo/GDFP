...
class Initial_GDFPBlock(nn.Module):
...
class Final_GDFP(nn.Module):

    def __init__(self,planes,layers,kernel=3,block=BasicBlock,inplanes = 3):
        super().__init__()
        self.block = GDFPBlock(planes,layers,kernel=kernel,block=block)

    def forward(self, x):
        out = self.block(x)
        return out[-1]

class GDFP(nn.Module):
    def __init__(self,inplanes=1,num_classes=2,layers=4,filters=10,):
        ...
        #self.middle_block = GDFPBlock(planes=filters,layers=layers)
        self.final_block = Final_GDFPB(planes=filters,layers=layers)
